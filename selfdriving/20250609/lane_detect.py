...
class SelfDrivingNode(Node):
    def __init__(self, name):
        ...
        ### if press 1 then 'enter' service call
...
###########################myadd######################################################################
    def button_callback(self, msg):
        if msg.id == 1:
            ...
            ## client service call
            req = Trigger.Request()
            self.exit_client.call_async(req)
            self.rgb_pub.publish(msg)
...
###############################add image callback function#################################
 ...
 ###########################################################################################       
    ...
    def park_action(self):
        ...
        ######################## not working because machine is MEntorPi_Mecanum################
        ...
        #     time.sleep(1.5)
        ##########################################################################################
        ################ i think this code will not execute#######################################
        else:
            twist = Twist()
            twist.angular.z = -1
            self.mecanum_pub.publish(twist)
        ...
        while self.is_running:
            ...
            if self.start:
                ...
                if 70 < self.crosswalk_distance and not self.start_slow_down:  # The robot starts to slow down only when it is close enough to the zebra crossing
                    ...
                    if self.count_crosswalk == 2:  # change                     
                        self.count_crosswalk = 0
