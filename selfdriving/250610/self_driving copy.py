...
class SelfDrivingNode(Node):
    def __init__(self, name):
        ...
        self.msg_on.states = [
                RGBState(index=1, red=255, green=0, blue=0),  # 1번 LED 빨강색
                RGBState(index=2, red=0, green=255, blue=0)   # 2번 LED 초록색
        ]
        self.msg_off.states = [
        ...
        ### if press 1 then exit
        self.enter_client = self.create_client(Trigger, '~/exit')
        ######
...
#############################################
    def button_callback(self, msg):
        ...
        if msg.id == 1:
            ...
            req = Trigger.Request()
            self.enter_client.call_async(req)
