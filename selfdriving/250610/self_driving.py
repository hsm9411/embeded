...
###############################################
    ...
    def param_init(self):
        ...
        self.normal_speed = 0.25  # normal driving speed
        ...
        while self.is_running:
            ...
            if self.start:
                ...
                if self.right_turn_state == "DETECTED":
                    if time.time() - self.right_detect_time >= 0.9:
                        self.turn_right_action()
                        self.right_turn_state = "TURNING"

                elif self.right_turn_state == "TURNING":
                    if time.time() - self.turn_right_start_time >= 1.7:
                        ...
                        self.get_logger().info("\033[1;32m[우회전 종료]\033[0m")
                ...
                if cross_detected and (current_time - self.last_crosswalk_pause_time > 4): #and not self.start_slow_down:  # The robot starts to slow down only when it is close enough to the zebra crossing
                    self.start = False  # 주행 정지
                    self.mecanum_pub.publish(Twist())  # 멈추기 위한 Twist 명령
                    self.get_logger().info('Driving has been stopped.')
                ...
                if False:#0 < self.park_x and 135 < self.crosswalk_distance:
                    twist.linear.x = self.slow_down_speed
                ...
                if lane_x >= 0 and not self.stop:  
                    if lane_x > 150: 
                        ...
                        if self.machine_type != 'MentorPi_Acker':
                            twist.angular.z = -0.8 #-0.45  # turning speed
                        else:
                            twist.angular.z = twist.linear.x * math.tan(-0.5061) / 0.145
        ...
        if self.objects_info == []:  # If it is not recognized, reset the variable
            ...
            if self.objects_info:
                ...
                if result_image is not None:
                    ...
                    # Create a filename with the current timestamp to avoid overwriting
                    timestamp = time.strftime("%Y%m%d_%H%M%S")
                    filename = os.path.join(self.IMAGE_SAVE_DIR, f"detected_{timestamp}.png")
