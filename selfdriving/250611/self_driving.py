...
class SelfDrivingNode(Node):
    ...
    def start_blinking_right_led(self, duration=3.0, interval=0.5):
        ...
        """
        self.get_logger().info("\033[1;34m[우회전 깜빡이 시작]\033[0m")
    ...
    def _blink_right_led_toggle(self, interval, duration):
        ...
        if current_time - self.blinking_start_time > duration:
            ...
            self.get_logger().info("\033[1;32m[우회전 깜빡이 종료]\033[0m")
            return
    ...
    def main(self):
        ...
        while self.is_running:
            ...
            if self.start:
                ...
                if (self.crosswalk_stop == True) and time.time() - self.crosswalk_stop_time <= 0.5:
                    twist.linear.x = 0.0
                    twist.angular.z = 0.0
                    self.mecanum_pub.publish(twist)
                    continue
                elif (self.crosswalk_stop == True) and time.time() - self.crosswalk_stop_time > 0.5:
                    self.crosswalk_stop = False
                    self.send_message("redoff")
                    self.send_message("greenon")
                ...
                if cross_detected and (current_time - self.last_crosswalk_pause_time > 5): #and not self.start_slow_down:  # The robot starts to slow down only when it is close enough to the zebra crossing
                    self.send_message("redon")
                    self.send_message("greenoff")
                    #self.start = False  # 주행 정지
                ...
                if self.traffic_signs_status is not None:
                    ...
                    if self.traffic_signs_status.class_name == 'red' and area > 600 and area < 1000:  # If the robot detects a red traffic light, it will stop
                        self.mecanum_pub.publish(Twist())
                        self.stop = True
                        self.send_message("redon")
                        self.send_message("greenoff")
                ...
                if lane_x >= 0 and not self.stop:  
                    if lane_x > 150: 
                        ...
                        if self.machine_type != 'MentorPi_Acker':
                            twist.angular.z = -0.65 #-0.45  # turning speed        
